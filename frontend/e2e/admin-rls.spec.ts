...
test.describe("Initiate Supabase", () => {
  ...
  test.beforeAll(async () => {
    ...
    const { data: existingUsers } = await supabaseSuperUser.auth.admin.listUsers();
    const adminUserToDelete = existingUsers?.users.find((user) => user.email === testAdminEmail);
    if (adminUserToDelete) {
      await supabaseSuperUser.auth.admin.deleteUser(adminUserToDelete.id);
    }
  });
  ...
  test.afterAll(async () => {
    ...
    const adminUsers = existingUsers?.users.filter((user) => user.user_metadata?.role === "admin") || [];
  ...
  test("Non admin user cannot access persona prompts", async ({ browserName }) => {
    // create a non admin user
    await supabaseSuperUser.auth.admin.createUser({
    ...
    const nonAdminUser = createClient(supabaseUrl, process.env.SUPABASE_ANON_KEY!);
    // login as the non admin user
    const { error: loginError } = await nonAdminUser.auth.signInWithPassword({
      email: "testuser@example.com",
      password: "testuserpassword",
    });
    if (loginError) {
    ...
    const { data: prompts, error: promptsError } = await nonAdminUser.from("persona_prompts").select("*");
    expect(prompts).toEqual([]);
    expect(promptsError).toBeNull();
  });
  ...
  test("Non admin user cannot access system prompts", async ({ browserName }) => {
    // create a non admin user
  await supabaseSuperUser.auth.admin.createUser({
      email: "testuser@example.com",
      password: "testuserpassword",
    ...
    const { data: prompts, error: promptsError } = await nonAdminUser.from("system_prompts").select("*");

    // For RLS restrictions, we expect data to be an empty array
    expect(prompts).toEqual([]);
